...
# Azerbaijani top artists/playlists — user requested
AZ_ARTIST_QUERIES = [
    \"Miri Yusif\",
    \"Aygün Kazımova\",
    \"Çakal rap\",
...
async def _warm_cache():
    \"\"\"Pre-fetch heavy queries so first visitor sees results instantly.\"\"\"
...
async def featured(category: str = \"azerbaijan top mahnilar 2025\"):
    if category not in FEATURED_QUERIES:
        # allow the query anyway, but don't spread our cache keys
        pass
...
async def az_top(limit: int = 20):
    \"\"\"Merged Azerbaijani top chart — instant via cache.\"\"\"
    merged: list = []
    seen = set()
...
async def home_bootstrap():
    \"\"\"One-shot endpoint that returns everything the home page needs.\"\"\"

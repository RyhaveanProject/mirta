...
const usePlayer = (toast) => {
  ...
  useEffect(() => {
    ...
    requestWakeLock();
    const onVisRelease = () => { if (document.visibilityState === "visible") requestWakeLock(); };
...
const SongCard = ({ song, onPlay, onLike, liked }) => (
  <div className="song-card" onClick={() => onPlay(song)}>
    ...
    {onLike && (
      <button className={`card-like-btn ${liked ? "liked" : ""}`}
...
const SongRow = ({ song, onPlay, rank, onLike, liked }) => (
  <div className={rank ? "row-ranked" : "song-row"} onClick={() => onPlay(song)}>
    ...
    {onLike && (
      <button className={`row-like-btn ${liked ? "liked" : ""}`}
        onClick={(e) => { e.stopPropagation(); onLike(song); }}>
        <Heart size={16} fill={liked ? "currentColor" : "none"} />
      </button>
    )}
...
const HomePage = ({ player, toggleFav, isFav }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/home-bootstrap`).then(({ data }) => {
      if (cancelled) return;
      setTopAz(data.top || []); setArtists(data.artists || []); setDiscovery(data.discovery || []); setLoading(false);
    }).catch(() => { if (!cancelled) setLoading(false); });
    axios.get(`${API}/recently-played?session_id=${player.sessionId}&limit=12`)
      .then(r => !cancelled && setRecent((r.data.recent || []).map(x => ({...x, id: x.song_id})))).catch(() => {});
    fetchTrending();
    window.addEventListener("fav_updated", fetchTrending);
    return () => { cancelled = true; window.removeEventListener("fav_updated", fetchTrending); };
    ...
    <div className="page">
      ...
      {recent.length > 0 && (
        <section className="section">
          ...
          <div className="row-scroll">{recent.slice(0, 10).map((s) => (<SongCard key={s.id} song={s} onPlay={player.play} onLike={toggleFav} liked={isFav(s.id)} />))}</div>
        </section>
      )}
      <section className="section">
...
const SearchPage = ({ player, toggleFav, isFav }) => {
  ...
  useEffect(() => { axios.get(`${API}/featured/categories`).then(r => setCats(r.data.categories || [])); }, []);
  const doSearch = useCallback(debounce(async (query) => {
    if (!query.trim()) { setResults([]); return; }
    setLoading(true);
    try { const { data } = await axios.get(`${API}/search?q=${encodeURIComponent(query)}&limit=20`); setResults(data.results || []); }
    catch { setResults([]); } finally { setLoading(false); }
  }, 350), []);

  useEffect(() => { doSearch(q); }, [q, doSearch]);
  const pickCategory = async (c) => {
    setActiveCat(c); setLoading(true);
    try { const { data } = await axios.get(`${API}/featured?category=${encodeURIComponent(c)}`); setCatResults(data.results || []); }
  ...
  return (
    <div className="page">
      ...
      <div className="search-wrap">
        ...
        <input className="search-input" placeholder="Mahnı, ifaçı..." value={q} onChange={(e) => setQ(e.target.value)} autoFocus />
        {q && <button className="search-clear-btn" onClick={() => setQ("")}><X size={16} /></button>}
      </div>
      {q.trim() === "" ? (
        <>
          <div className="chip-row">{cats.map((c) => (<div key={c} className={`chip ${activeCat === c ? "active" : ""}`} onClick={() => pickCategory(c)}>{c}</div>))}</div>
          {activeCat && <div className="song-list">{catResults.map((s) => <SongRow key={s.id} song={s} onPlay={player.play} onLike={toggleFav} liked={isFav(s.id)} />)}</div>}
        </>
      ) : (
        <div className="song-list">{loading ? [...Array(6)].map((_, i) => <Skeleton key={i} h="58px" />) : results.map((s) => <SongRow key={s.id} song={s} onPlay={player.play} onLike={toggleFav} liked={isFav(s.id)} />)}</div>
      )}
    </div>
  );
};
...
const FavoritesPage = ({ player, favs, toggleFav, isFav }) => {
  const [trending, setTrending] = useState([]);
  const fetchTrending = useCallback(() => { axios.get(`${API}/trending?limit=10`).then(r => setTrending(r.data.trending || [])).catch(() => {}); }, []);
  useEffect(() => { fetchTrending(); window.addEventListener("fav_updated", fetchTrending); return () => window.removeEventListener("fav_updated", fetchTrending); }, [fetchTrending]);

  return (
    <div className="page">
      <h1 className="page-title vip-title">My List</h1>
      <div className="page-sub vip-subtitle">{favs.length} bəyənilmiş mahnı</div>
      <div className="song-list">{favs.map((s) => <SongRow key={s.song_id} song={{id: s.song_id, ...s}} onPlay={player.play} onLike={toggleFav} liked={isFav(s.song_id)} />)}</div>
      <section className="section">
        <div className="section-title vip-section"><TrendingUp size={16} style={{marginRight:6, display:"inline"}} /> Top Trending</div>
        <div className="song-list">{trending.map((s, i) => <SongRow key={s.id} song={s} onPlay={player.play} rank={i + 1} onLike={toggleFav} liked={isFav(s.id)} />)}</div>
      </section>
    </div>
  );
};
  ...
  return (
    <div className="full-player">
      ...
      <div className="full-body">
        <div className="art-wrap"><div className={`art-inner ${player.playing ? "playing" : ""}`} style={{ backgroundImage: `url(${s.thumbnail})` }} /></div>
        <div className="track-info">
          <div className="t">{s.title}</div><div className="a">{s.artist}</div>
          {player.playing && <div className="visualizer" style={{justifyContent:"center", marginTop:12}}><span /><span /><span /><span /></div>}
        </div>
        <div className="progress-wrap">
          <div className="progress-bar" onClick={(e) => { const r = e.currentTarget.getBoundingClientRect(); player.seek((e.clientX - r.left) / r.width); }}>
            <div className="fill" style={{ width: `${pct * 100}%` }} /><div className="knob" style={{ left: `${pct * 100}%` }} />
          </div>
          <div className="time-row"><span>{fmtTime(player.progress)}</span><span>{fmtTime(player.duration || s.duration)}</span></div>
        </div>
        <div className="controls-row">
          ...
          <button className="ctrl play" onClick={player.togglePlay}>{player.loadingStream ? <Loader2 size={24} className="animate-spin" /> : player.playing ? <Pause size={26} fill="currentColor" /> : <Play size={26} fill="currentColor" />}</button>
          <button className="ctrl" onClick={player.next}><SkipForward size={22} fill="currentColor" /></button>
          <button className={`ctrl ${player.repeat !== "off" ? "active" : ""}`} onClick={() => { const order = ["off", "all", "one"]; player.setRepeat(order[(order.indexOf(player.repeat) + 1) % 3]); }}>{player.repeat === "one" ? <Repeat1 size={18} /> : <Repeat size={18} />}</button>
        </div>
        <div className="extra-row">
          <button className={`ctrl like-btn ${liked ? "liked heart-burst" : ""}`} onClick={() => toggleFav(s)}><Heart size={20} fill={liked ? "currentColor" : "none"} /></button>
          <button className="ctrl" onClick={() => player.enqueue(s)}><Plus size={20} /></button>
          <button className="ctrl"><ListMusic size={20} /></button>
        </div>
        <div className="vol-wrap"><Volume2 size={16} /><div className="vol-bar" onClick={(e) => { const r = e.currentTarget.getBoundingClientRect(); player.setVolume((e.clientX - r.left) / r.width); }}><div className="fill" style={{ width: `${player.volume * 100}%` }} /></div></div>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="mini-player" onClick={() => player.setFullOpen(true)}>
      ...
      <div className="controls" onClick={(e) => e.stopPropagation()}>
        <button className={`ctrl-btn ${isFav(player.current.id) ? "liked" : ""}`} onClick={() => toggleFav(player.current)}><Heart size={18} fill={isFav(player.current.id) ? "currentColor" : "none"} /></button>
        <button className="ctrl-btn" onClick={player.togglePlay}>{player.loadingStream ? <Loader2 size={18} className="animate-spin" /> : player.playing ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />}</button>
...
const BottomNav = ({ tab, setTab }) => {
  const items = [{ id: "home", label: "Home", Icon: HomeIcon }, { id: "search", label: "Search", Icon: SearchIcon }, { id: "favs", label: "My List", Icon: Heart }, { id: "nowp", label: "Player", Icon: Music2 }];
  return (<nav className="bottom-nav">{items.map(({ id, label, Icon }) => (<button key={id} className={`nav-btn ${tab === id ? "active" : ""}`} onClick={() => setTab(id)}><Icon size={22} /><span>{label}</span></button>))}</nav>);
};

function App() {
  const toast = useToast(); const player = usePlayer(toast); const [tab, setTab] = useState("home"); const [favs, setFavs] = useState([]); const [likePending, setLikePending] = useState({});
  useDynamicBg(player.current);
  const refreshFavs = useCallback(() => { axios.get(`${API}/favorites?session_id=${player.sessionId}`).then(r => setFavs(r.data.favorites || [])).catch(() => {}); }, [player.sessionId]);
  ...
  const toggleFav = useCallback(async (s) => {
    if (!s || !s.id || likePending[s.id]) return;
    setLikePending(p => ({ ...p, [s.id]: true }));
    const currentlyFav = isFav(s.id);
    try {
      if (currentlyFav) { await axios.delete(`${API}/favorites/${s.id}?session_id=${player.sessionId}`); toast.show("Silindi"); }
      else { await axios.post(`${API}/favorites`, { session_id: player.sessionId, song: { id: s.id, ...s } }); toast.show("❤️‍🔥"); }
    } finally { setLikePending(p => { const { [s.id]: _, ...rest } = p; return rest; }); refreshFavs(); window.dispatchEvent(new Event("fav_updated")); }
  }, [isFav, likePending, player.sessionId, refreshFavs, toast]);

  useEffect(() => { if (tab === "nowp") { if (player.current) player.setFullOpen(true); else toast.show("Mahnı seçin"); setTab("home"); } }, [tab, player, toast]);

  return (
    <div className="app-shell"><div className="dynamic-bg" /><BrandHeader />
      {tab === "home" ? <HomePage player={player} toggleFav={toggleFav} isFav={isFav} /> :
       ...
       tab === "favs" ? <FavoritesPage player={player} favs={favs} toggleFav={toggleFav} isFav={isFav} /> : null}
      <MiniPlayer player={player} toggleFav={toggleFav} isFav={isFav} /><BottomNav tab={tab} setTab={setTab} />
      {player.fullOpen && <FullPlayer player={player} toggleFav={toggleFav} isFav={isFav} />}{toast.node}
    </div>
  );
}

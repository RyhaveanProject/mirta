...
import DownloadManager, { onProgress as onDlProgress, offProgress as offDlProgress } from "./downloadManager";
import "./App.css";
...
import {
  ...
  Download, CheckCircle2, Trash2, WifiOff
} from "lucide-react";
...
const usePlayer = (toast) => {
  ...
  const play = useCallback(async (song, opts = {}) => {
    ...
    // Direct stream by default → device fetches audio bytes from
    // googlevideo using its OWN IP (not server proxy). Falls back to
    // proxy=1 if direct attach fails (e.g. CDN refuses cellular IP).
    axios.get(`${API}/stream/${song.id}`).then(async ({ data }) => {

  if (data?.stream_url) {
    let ok = await AudioManager.attachStream(data.stream_url, {
      title: song.title,
      artist: song.artist,
    ...
    if (!ok) {
      try {
        const fb = await axios.get(`${API}/stream/${song.id}?proxy=1`);
        if (fb?.data?.stream_url) {
          await AudioManager.attachStream(fb.data.stream_url, {
            title: song.title,
            artist: song.artist,
            thumbnail: song.thumbnail,
          });
...
const useDownloadState = (songId) => {
  ...
  useEffect(() => {
    let cancelled = false;
    if (!songId) { setDownloaded(false); return; }
    DownloadManager.isDownloaded(songId).then((d) => { if (!cancelled) setDownloaded(d); });
    return () => { cancelled = true; };
  }, [songId]);

  useEffect(() => {
    const cb = (p) => {
      if (p.id !== songId) return;
      if (p.done) {
        setDownloaded(true);
        setProgress(null);
        setBusy(false);
    ...
    onDlProgress(cb);
    return () => offDlProgress(cb);
  }, [songId]);

  return { downloaded, progress, busy, setBusy, setDownloaded };
};

const DownloadButton = ({ song, toast, size = 20, className = "ctrl" }) => {
  const { downloaded, progress, busy, setBusy, setDownloaded } = useDownloadState(song?.id);
  if (!song) return null;
  const handle = async (e) => {
    e.stopPropagation();
    if (busy || downloaded) {
      if (downloaded) {
        await DownloadManager.removeDownload(song.id);
        setDownloaded(false);
        toast.show("Endirilmiş silindi");
      }
      return;
    }
    setBusy(true);
    try {
      ...
      toast.show("Mahnı endirildi — offline çalmaq mümkündür");
    } catch (err) {
      toast.show("Endirmə alınmadı");
      setBusy(false);
    }
  };
  const label = downloaded ? "Endirilib" : (busy ? `${progress ?? 0}%` : "Endir");
  return (
    <button
      className={`${className} ${downloaded ? "active" : ""}`}
      onClick={handle}
      data-testid={`download-btn-${song.id}`}
      aria-label={label}
      title={label}
    >
      {busy ? <Loader2 size={size} className="animate-spin" /> :
        downloaded ? <CheckCircle2 size={size} /> : <Download size={size} />}
    </button>
  );
};

/* ---------- Full Player ---------- */
const FullPlayer = ({ player, toggleFav, isFav, toast }) => {
    ...
    <div className="full-player" data-testid="full-player">
      ...
      <div className="full-body">
        ...
        <div className="extra-row">
          ...
          <DownloadButton song={s} toast={toast} size={20} />
    ...
    <div className={`welcome-screen ${leaving ? "leaving" : ""}`} data-testid="welcome-screen">
      ...
      <div className="welcome-bottom">
        ...
        <div className="welcome-foot">
          By <span className="gold">Ryhavean</span> 
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  else if (tab === "downloads") pageEl = <DownloadsPage player={player} toast={toast} />;
  ...
  return (
    <div className="app-shell">
      ...
      {player.fullOpen && <FullPlayer player={player} toggleFav={toggleFav} isFav={isFav} toast={toast} />}
      {toast.node}
